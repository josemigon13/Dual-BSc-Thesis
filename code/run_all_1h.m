...
% Compresión directa [1h]
Xc = padarray(X, [ceil(size(Xsc, 1)/60)*60-size(Xsc, 1), 0], 0, 'post');
Xc = squeeze(sum(reshape(Xc,60,[], size(Xc, 2)), 1));

Yc = padarray(Y, [ceil(size(Ysc, 1)/60)*60-size(Ysc, 1), 0], 0, 'post');
Yc = squeeze(sum(reshape(Yc,60,[], size(Yc, 2)), 1));
...
% Compresión directa [1h]
testc = padarray(test, [ceil(size(testsc, 1)/60)*60-size(testsc, 1), 0], 0, 'post');
testc = squeeze(sum(reshape(testc,60,[], size(testc, 2)), 1));

Ytc = padarray(Yt, [ceil(size(Ytsc, 1)/60)*60-size(Ytsc, 1), 0], 0, 'post');
Ytc = squeeze(sum(reshape(Ytc,60,[], size(Ytc, 2)), 1));
...
for i=0:size(Xc_p,1)-2
    XX = Xsc_p(i*60+1:i*60+60,:)'*Xsc_p(i*60+1:i*60+60,:);
    Xa(i+1,:) = ADICOV(XX,Xc_p(i+1,:),1,P);
    Xr(i+1,:) = ADICOV(XX,Xc_p(i+1,:),268-1,Pr);
end

% Última iteración
XX = Xsc_p(72601:72644,:)'*Xsc_p(72601:72644,:);
Xa(1211,:) = ADICOV(XX,Xc_p(1211,:),1,P);
Xr(1211,:) = ADICOV(XX,Xc_p(1211,:),268-1,Pr);
tic
[~,~,Dstt,Qstt,UCLd,UCLq] = mspc_pca(Xa+Xr,1:1,testsc_p,0,000);
toc
pred = 1/268*Dstt/UCLd(1) + (268-1)/268*Qstt/UCLq(1);
