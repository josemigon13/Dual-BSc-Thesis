...
% Compresión directa [1 día]
Xc = padarray(X, [ceil(size(Xsc, 1)/1440)*1440-size(Xsc, 1), 0], 0, 'post');
Xc = squeeze(sum(reshape(Xc,1440,[], size(Xc, 2)), 1));

Yc = padarray(Y, [ceil(size(Ysc, 1)/1440)*1440-size(Ysc, 1), 0], 0, 'post');
Yc = squeeze(sum(reshape(Yc,1440,[], size(Yc, 2)), 1));
...
% Compresión directa [1 día]
testc = padarray(test, [ceil(size(testsc, 1)/1440)*1440-size(testsc, 1), 0], 0, 'post');
testc = squeeze(sum(reshape(testc,1440,[], size(testc, 2)), 1));

Ytc = padarray(Yt, [ceil(size(Ytsc, 1)/1440)*1440-size(Ytsc, 1), 0], 0, 'post');
Ytc = squeeze(sum(reshape(Ytc,1440,[], size(Ytc, 2)), 1));
...
for i=0:size(Xc_p,1)-2
    XX = Xsc_p(i*1440+1:i*1440+1440,:)'*Xsc_p(i*1440+1:i*1440+1440,:);
    Xa(i+1,:) = ADICOV(XX,Xc_p(i+1,:),1,P);
    Xr(i+1,:) = ADICOV(XX,Xc_p(i+1,:),268-1,Pr);
end

% Última iteración
XX = Xsc_p(72001:72644,:)'*Xsc_p(72001:72644,:);
Xa(51,:) = ADICOV(XX,Xc_p(51,:),1,P);
Xr(51,:) = ADICOV(XX,Xc_p(51,:),268-1,Pr);
tic
[~,~,Dstt,Qstt,UCLd,UCLq] = mspc_pca(Xa+Xr,1:1,testsc_p,0,000);
toc
pred = 1/268*Dstt/UCLd(1) + (268-1)/268*Qstt/UCLq(1);
